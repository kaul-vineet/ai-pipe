...
def calculatemetrics():
    ...
    cur = conn.cursor()
    try:
        cur.execute("INSERT INTO public.modelmetrics(model_score, model_timestamp, model_recordcount) VALUES (%s, %s, %s)", (truncate(score, 3), str(datetime.datetime.now()), int(dfsize)))
        conn.commit()
